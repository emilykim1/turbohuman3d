...
def main(args):
    ...
    if args.pretrained_model_name_or_path == "stabilityai/sd-turbo":
        ...
        if args.pretrained_model_name_or_path[-3:] == 'pkl':
            net_pix2pix = Pix2Pix_Turbo(pretrained_name='', pretrained_path=args.pretrained_model_name_or_path)
            net_pix2pix.set_train()
        ...
        for step, batch in enumerate(dl_train):
            ...
            with accelerator.autocast():
                with accelerator.accumulate(*l_acc):
                    ...
                    if step > 5000:
                        arc_weight = 0.3  # identity matters more now
                        style_weight = 2.0
                    ...
                    loss_l2 = F.mse_loss(x_tgt_pred.float(), x_tgt[:, 1].reshape(B, C, H, W).float(), reduction="mean") 
                    loss_lpips = net_lpips(x_tgt_pred.float(), x_tgt[:, 1].reshape(B, C, H, W).float()).mean() 
                    ...
                    style_loss = torch.tensor(0.0, requires_grad=True).cuda()
                    
                    if global_step > 0:
                    
                        with torch.no_grad():
                            tgt = img_transform(x_tgt[:,1])
                            pred = img_transform(x_tgt_pred)
                            ...
                            for b in range(x_tgt.shape[0]):
                                ...
                                x_tgt_np = einops.rearrange(x_tgt_pred[b].float().detach().cpu().numpy(), 'c h w -> h w c')
                                x_tgt_np = ((x_tgt_np + 1) * 127.5).astype(np.uint8)
                                ...
                                if len(face_tgt) == 0 or len(face_pred) == 0:
                                    ...
                                    face_crop_tgt = face_crop.to(dtype=torch.bfloat16) * torch.bernoulli(torch.full((B, 1, 1, 1), 0.9)).cuda()
                                    ...
                                    x1, y1, x2, y2 = bbox
                                    bbox_norm = torch.tensor([x1/W_1, y1/H_1, x2/W_1, y2/H_1]).cuda()
                                    face_crop = crop_face_affine(x_tgt_pred[[b]].float(), bbox_norm)
                    ...
                    if global_step > 100:
                        loss += loss_lpips * lpips_weight
                        loss += style_loss * style_weight + loss_arc * arc_weight

                    if global_step % args.eval_freq == 1:
                        os.makedirs(os.path.join(args.output_dir, "train", f"fid_{global_step}"), exist_ok=True)
                        output_img = torch.cat([x_tgt[0][2], x_tgt[0][0], x_tgt[0][1], x_src[0][1], x_tgt_pred[0]], dim=2)
                    ...
                    lossG = net_disc(x_tgt_pred.reshape(B, C, H, W), for_G=True).mean() * args.lambda_gan
                    ...
                    # fake image
                    lossD_fake = net_disc(x_tgt_pred.detach().reshape(B, C, H, W), for_real=False).mean() * args.lambda_gan
                        ...
                        if global_step % args.eval_freq == 1:
                            ...
                            for step, batch_val in enumerate(dl_val):
                                ...
                                with torch.no_grad():
                                    ...
                                    loss_l2 = F.mse_loss(x_tgt_pred.float(), x_tgt[:, 1].float(), reduction="mean")
                                    loss_lpips = net_lpips(x_tgt_pred.float(), x_tgt[:, 1].float()).mean()
                                ...
                                output_img = torch.cat([x_tgt[0][1], x_src[0][1], x_tgt_pred[0]], dim=2)
